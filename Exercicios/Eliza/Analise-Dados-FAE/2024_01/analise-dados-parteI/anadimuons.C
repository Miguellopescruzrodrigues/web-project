  ...
  for (Long64_t jentry=firstEntry; jentry<=lastEntry;jentry++) {
    ...
    nb = fChain->GetEntry(jentry);   nbytes += nb;
      ...
      //CORTES
        ...
        // Cortes mu1
        if (muonP_p4->Pt() < pTMin || fabs(muonP_p4->Eta()) > etaMax) {
            passCuts = false;
        }

        // cortes mu2
        if (muonN_p4->Pt() < pTMin || fabs(muonN_p4->Eta()) > etaMax) {
            passCuts = false;
        }
...
void anadimuon::FitPeakRoofit()
{
  ...
  RooCBShape crystalball("CBS", "CBS", mass, mean, sigma, alpha, n); //-----------------------------------

  RooBreitWigner BW("BW","BW",mass,mean,sigma); //----------------------------------
  ...
  RooRealVar frac1("frac1","frac1",0.46);
  RooAddPdf* signal;
  ...
  L.SetTextSize(0.03);
  L.DrawLatex(0.15,0.75,"resonance: Z^{0}");
  ...
  L.DrawLatex(0.15,0.60,Form("yield: %.0f #pm %.0f events", 
    ...
    L.DrawLatex(0.15,0.55,Form("#Chi^{2}: %2.5f", 
           frame->chiSquare()));
  L.DrawLatex(0.15,0.50,"Corte: p_{T}#geq 8 e |#eta|#leq 2,4");
...
void anadimuon::FitPeak()
{
  ...
  L.DrawLatex(0.15,0.60,"resonance: J/#psi");
